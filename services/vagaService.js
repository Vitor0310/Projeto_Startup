...
// NOVA FUNÇÃO para buscar vagas POR DONO (Locador)
export async function getVagasByOwner(userId) {
  try {
    const vagasCollection = collection(db, "vagas");
    // Cria a consulta para buscar documentos onde 'locadorId' == userId
    const q = query(vagasCollection, where("locadorId", "==", userId));
    ...
    console.error("Erro ao buscar vagas do locador: ", e);
    return [];
  }
}